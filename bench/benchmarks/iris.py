...
CLASSES = {
    ...
    'ITMLTransformer': metric_learn.ITMLTransformer(num_constraints=200),
    'LFDA': metric_learn.LFDA(k=2, dim=2),
    'LMNN': metric_learn.LMNN(k=5, learn_rate=1e-6, verbose=False),
    'LSMLTransformer': metric_learn.LSMLTransformer(num_constraints=200),
    'MLKR': metric_learn.MLKR(),
    'NCA': metric_learn.NCA(max_iter=700, learning_rate=0.01, num_dims=2),
    'RCATransformer': metric_learn.RCATransformer(dim=2, num_chunks=30,
                                                  chunk_size=2),
    'SDMLTransformer': metric_learn.SDMLTransformer(num_constraints=1500),
}
